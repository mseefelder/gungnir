...
public:
    ...
    void initialize(int width, int height)
    {
	    ...
	    rect.initialize();
	    wisard.setShadersDir(shaders_dir);
	    wisard.initialize();
    ...
    virtual void paintGL(cv::Mat* frame)
    {
	    ...
	    if(ROIDefined)
	    {
	        if(!qValueReady)
	        {
	            wisard.startTracking(viewport, ROIcorner, ROIspread, frameTexture, NREG);
	            qValueReady = true;
	        }
	        else
	        {
	            wisard.track(ROIcorner);//frameTexture, &ROIcorner, &ROIspread, 100, frameNorm);
	        }
	    }
	...
	void updateROI(Eigen::Vector2i &current)
	{
		...
		{
			ROIcorner = current;
			ROIspread = Eigen::Vector2i(current[0]+ROIsize, current[1]+ROIsize);
		}
		/**
		if(markROI)
			ROIspread = current;
